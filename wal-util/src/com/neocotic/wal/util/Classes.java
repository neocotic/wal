...
public class Classes
{
    ...
    public static RuntimeLocator getRuntimeLocator()
    {
        return getRuntimeLocator(0);
    }

    public static RuntimeLocator getRuntimeLocator(int offset)
    {
        if (offset < 0)
        {
            offset = 0;
        }
        return new RuntimeLocator(++offset);
    }
    ...
    public static boolean isExtensionOf(Class<?> cls, Class<?> spr)
    {
        ...
        if (spr == null && superClass == null) { return true; }
    ...
    public static boolean isImplementationOf(Class<?> cls, Class<?>... interfaces)
    {
        ...
        if (Arrays.isEmpty(interfaces) && Arrays.isEmpty(actualInterfaces)) { return true; }
    ...
    protected static class RuntimeLocator
    {
        ...
        private RuntimeLocator(int offset)
        {
            if (offset < 1)
            {
                offset = 1;
            ...
            StackTraceElement element = new Exception().getStackTrace()[++offset];
            currentClassName = element.getClassName();
        ...
        public Class<?> getCurrentClass()
        {
            if (currentClass == null)
            {
                try
                {
                    currentClass = ClassLoader.getSystemClassLoader().loadClass(currentClassName);
                }
                catch (Exception e)
                {}
            }
